...
if [ "$i" != "--tests" ];then
 ...
 nohup java -jar ${ARGS} ${sitoolsCore}/${sitoolsJarName} 2>&#38;1 | tee -a ${LOG} &#38;
else
...
:: Lancement de JAVA
SET ARGS=-Xms256m -Xmx512m -Djava.net.preferIPv4Stack=true -Djava.util.logging.config.file=%sitoolsCore%/conf/properties/sitools-logging.properties
IF "%1"=="--tests" GOTO tests
	...
	java -jar %sitoolsSnap%/sitools-update-classpath/sitools-update-classpath.jar --tmp_directory=ext --directory=ext --jar_target=fr.cnes.sitools.core.jar 2>&amp;1 >> %LOG%
  	...
  	java -jar %ARGS% fr.cnes.sitools.core.jar >> %LOG% 2>&amp;1
  	GOTO :EOF
:tests
