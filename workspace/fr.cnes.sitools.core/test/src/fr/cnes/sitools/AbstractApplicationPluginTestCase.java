...
public abstract class AbstractApplicationPluginTestCase extends AbstractSitoolsServerTestCase {
  ...
  public void testCRUD() {
    ...
    try {
      ...
      assertNone();
      // create object
      ApplicationPluginModelDTO model = createObject("100000");
      ...
      stop(model);
      // check that the application is correctly stopped
  ...
  public void testCRUDdocAPI() {
    ...
    try {
      ...
      ApplicationPluginModelDTO appModel = createObjectForValidation("app_description_1", "100000");
  ...
  private void retrieveAllApp(int numberOfApplications) {
    ...
    if (docAPI.isActive()) {
      ...
      assertEquals(numberOfApplications, response.getTotal().intValue());
      RIAPUtils.exhaust(result);
    }
  ...
  private void getApplicationByName(ApplicationPluginModelDTO model) throws IOException {
    if (docAPI.isActive()) {
      Map<String, String> parameters = new LinkedHashMap<String, String>();
      retrieveDocAPI(getBaseUrl() + "/classes/" + model.getClassName(), "", parameters, getBaseUrl() + "/classes/"
          + model.getClassName());
    }
    else {
  ...
  private void getApplicationById(ApplicationPluginModelDTO model) throws IOException {
    if (docAPI.isActive()) {
      Map<String, String> parameters = new LinkedHashMap<String, String>();
      retrieveDocAPI(getBaseUrl() + "/instances/" + model.getId(), "", parameters,
          getBaseUrl() + "/instances/" + model.getId());
    }
    else {
    ...
    try {
      ...
      if (isArray) {
        ...
        if (media.isCompatible(MediaType.APPLICATION_JSON)) {
          ...
          xstream
              .addImplicitCollection(ApplicationPluginModelDTO.class, "parameters", ApplicationPluginParameter.class);
        ...
        if (dataClass == ApplicationPluginModelDTO.class) {
          ...
          if (media.isCompatible(MediaType.APPLICATION_JSON)) {
            xstream.addImplicitCollection(ApplicationPluginModelDTO.class, "parameters",
                ApplicationPluginParameter.class);
          }
        }
  ...
  public static Representation getRepresentation(ApplicationPluginModelDTO item, MediaType media) {
    if (media.equals(MediaType.APPLICATION_JSON)) {
      ...
      XstreamRepresentation<ApplicationPluginModelDTO> rep = new XstreamRepresentation<ApplicationPluginModelDTO>(
          media, item);
      configure(xstream);
      rep.setXstream(xstream);
      return rep;
