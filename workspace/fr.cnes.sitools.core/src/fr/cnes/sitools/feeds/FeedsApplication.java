 ...
 * 
 * TODO Constructor with all generic security configuration (Authenticator
 * informations) Configure security application by spring or from server main
 * 
 * @author AKKA
 * 
 */
public class FeedsApplication extends SitoolsApplication {
   ...
   * 
   */
  public FeedsApplication(Context context, FeedsStore store) {
    super(context);
    this.store = store;
   ...
   * @return the store
   */
  public final FeedsStore getStore() {
    return store;
  }
   ...
   *          the store to set
   */
  public final void setStore(FeedsStore store) {
    this.store = store;
  }
