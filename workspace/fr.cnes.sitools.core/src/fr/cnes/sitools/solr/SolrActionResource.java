...
public final class SolrActionResource extends AbstractSolrResource {
  ...
  private Response createIndex(Representation entity) {
    ...
    if (entity.getMediaType().isCompatible(MediaType.APPLICATION_JAVA_OBJECT)) {
      try {
        ...
        DataConfigDTO dataConfig = solR.getDataConfigDTO();
   ...
   */
  private boolean createDataConfigFile(DataConfigDTO dataConfig, String solRIndexConfDir) {
    try {
      ...
      File file = new File(solRIndexConfDir, "db-data-config.xml");
