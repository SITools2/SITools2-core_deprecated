...
import fr.cnes.sitools.feeds.FeedsStore;
...
public final class Starter {
  ...
  public static void start(String hostname, int port, String publicHostName) throws Exception {
    ...
    try {
      stores = StoreHelper.initContext(settings);
    }
    ...
    // Realm
    SitoolsMemoryRealm smr = new SitoolsMemoryRealm(storeUandG, storeRole, settings);
    ...
    // Store
    ConverterStore storeConv = (ConverterStore) settings.getStores().get(Consts.APP_STORE_DATASETS_CONVERTERS);
    ...
    // Store
    FilterStore storeFilter = (FilterStore) settings.getStores().get(Consts.APP_STORE_DATASETS_FILTERS);
    ...
    // Store
    DataSetStore storeDataSet = (DataSetStore) settings.getStores().get(Consts.APP_STORE_DATASET);
    ...
    // Store
    SitoolsStore<ProjectModule> storeProjectModules = (SitoolsStore<ProjectModule>) settings.getStores().get(
    ...
    // Store
    FormStore storeForm = (FormStore) settings.getStores().get(Consts.APP_STORE_FORM);
    ...
    // Store
    FeedsStore storeFeeds = (FeedsStore) settings.getStores().get(Consts.APP_STORE_FEED);
    ...
    // Store
    OpenSearchStore storeOS = (OpenSearchStore) settings.getStores().get(Consts.APP_STORE_OPENSEARCH);
    ...
    appContext.getAttributes().put(ContextAttributes.APP_ATTACH_REF, appReference);
    appContext.getAttributes().put(ContextAttributes.APP_REGISTER, true);
    appContext.getAttributes().put("USER_STORAGE_ROOT", settings.getStoreDIR(Consts.USERSTORAGE_ROOT));
    ...
    appContext.getAttributes().put(ContextAttributes.APP_REGISTER, true);
    appContext.getAttributes().put(ContextAttributes.APP_STORE, storeUS);
    appContext.getAttributes().put("USER_STORAGE_ROOT", settings.getStoreDIR("Starter.USERSTORAGE_ROOT"));
