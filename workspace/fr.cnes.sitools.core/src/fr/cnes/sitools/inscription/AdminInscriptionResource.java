...
public final class AdminInscriptionResource extends InscriptionResource {
  ...
  public Representation putInscription(Representation representation, Variant variant) {
    ...
    if (representation == null) {
      ...
      if (loginHasChanged && (inscriptionInput.getPassword() == null || inscriptionInput.getPassword().equals(""))) {
        if (variant.getMediaType().isCompatible(MediaType.APPLICATION_JSON) || variant.getMediaType().isCompatible(MediaType.APPLICATION_XML)) {
    ...
    if (validating) {
      ...
      if (myObj.isSuccess() && myObj.getItem() != null) {
        ...
        String templatePath = settings.getRootDirectory() + settings.getString(Consts.TEMPLATE_DIR) + "mail.account.activated.ftl";
        ...
        try {
          ...
          Request request = new Request(Method.POST, RIAPUtils.getRiapBase() + settings.getString(Consts.APP_MAIL_ADMIN_URL),  new ObjectRepresentation<Mail>(mailToUser));
 ...
 // url de la nouvelle resource en retour ?
    ...
    if (MediaType.APPLICATION_XML.isCompatible(variant.getMediaType())
      ...
      XStream xstream = XStreamFactory.getInstance().getXStreamWriter(variant.getMediaType(), false);
      configure(xstream, response);
      XstreamRepresentation<Response> xsRepresentation = new XstreamRepresentation<Response>(variant.getMediaType(),
          response);
      xsRepresentation.setXstream(xstream);
      return xsRepresentation;
    }
