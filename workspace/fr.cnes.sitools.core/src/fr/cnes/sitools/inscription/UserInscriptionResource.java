...
public final class UserInscriptionResource extends InscriptionResource {
  ...
  public Representation post(Representation representation, Variant variant) {
    ...
    try {
      ...
      XStream xstreamWriter = XStreamFactory.getInstance().getXStreamWriter(variant.getMediaType(), true);
      configure(xstreamWriter, response);

      XstreamRepresentation<Response> rep = new XstreamRepresentation<Response>(variant.getMediaType(), response);
      rep.setXstream(xstreamWriter);
      return rep;
    }
