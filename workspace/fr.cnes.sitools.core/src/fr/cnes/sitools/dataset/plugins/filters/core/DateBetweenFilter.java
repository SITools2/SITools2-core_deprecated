...
public final class DateBetweenFilter extends AbstractFormFilter {
  ...
  public DateBetweenFilter() {
    ...
    paramInfo = new ParameterInfo("p[#]", false, "xs:string", ParameterStyle.QUERY,
        "DATE_BETWEEN|columnAlias|valueFrom|valueTo");
    rpd.put("0", paramInfo);
    paramInfo = new ParameterInfo("c[#]", false, "xs:string", ParameterStyle.QUERY,
        "DATE_BETWEEN|dictionaryName,conceptName|valueFrom|valueTo");
    rpd.put("1", paramInfo);
   ...
   * @return true if values agree, false otherwise
   * @throws ParseException
   *           if the Date values are not in the correct format
   * @throws SitoolsException
   *           if the from date is not before the To date
   */
  private boolean checkValues(String[] parameters) throws ParseException, SitoolsException {
    values = Arrays.copyOfRange(parameters, VALUES, parameters.length);
    if (values.length == 2) {
      Date date1 = DateUtils.parse(values[0]);
      Date date2 = DateUtils.parse(values[1]);
      if (date1.before(date2) || date1.equals(date2)) {
        return true;
      }
      else {
        throw new SitoolsException("From date must be before To date");
      }
    }
    return false;
  }
