...
public class DataSetExplorerUtil {
  ...
  public final String getOrderBy() {
    ...
    try {
      ...
      Multisort orders = mapper.readValue(sortParam, Multisort.class);

      // if the order list is empty, there are no order by to add
      if (orders.getOrdersList().length == 0) {
        return "";
      }
      ...
      for (int i = 0; i < orders.getOrdersList().length; i++) {
        ...
        result += glue + " " + order.getField() + " " + order.getDirection();
        glue = ", ";
      }
      return result;
    }
      ...
      String sortParam = this.pagination.getFirstValue("sort", true);
      if ((sortParam != null) && !sortParam.equals("")) {
        return " ORDER BY " + sortParam + " " + getSortDirection();
      }
    }
    return "";
  ...
  public final ArrayList<Predicat> getPredicats() {
    ...
    if (this.pagination.getFirstValue("query") != null) {
      ...
      predicat.setCompareOperator(" like ");
