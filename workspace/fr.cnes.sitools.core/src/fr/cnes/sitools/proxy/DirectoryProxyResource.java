 ...
 */
public final class DirectoryProxyResource extends AbstractDirectoryServerResource {
  ...
  public void doInit() {
    try {
      ...
      // PATCH SITOOLS JPB (DirectoryProxy) au lieu de (Directory)
      this.setDirectory((DirectoryProxy) getRequestAttributes().get("org.restlet.directory"));
        ...
        if (contextResponse.getEntity() != null) {
          ...
          if (MediaType.TEXT_URI_LIST.equals(contextResponse.getEntity().getMediaType())) {
            ...
            if (!this.getTargetUri().endsWith("/")) {
              this.setTargetUri(this.getTargetUri().concat("/"));
  ...
  public List<Variant> getVariants(Method method) {
    ...
    if (!isIntranet) {
      ...
      DirectoryProxy proxyDirectory = (DirectoryProxy) getDirectory();
      if (proxyDirectory.getProxyBaseRef() != null && publicHostDomain != null && !"".equals(publicHostDomain)) {
        ...
        getReference().setBaseRef(publicHostDomain + proxyDirectory.getProxyBaseRef());
      }
