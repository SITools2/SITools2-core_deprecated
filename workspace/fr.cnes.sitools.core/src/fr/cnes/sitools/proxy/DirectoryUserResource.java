...
public final class DirectoryUserResource extends AbstractDirectoryServerResource {
  ...
  /**
   * Difference with DirectoryProxyResource : Can recursively delete directory with request parameter recursive=true
   * 
    ...
    if (this.getDirectory().isModifiable()) {
      ...
      if (this.isDirectoryTarget() && !this.indexTarget) {
        ...
        if ((getRequest().getResourceRef() != null) && (getRequest().getResourceRef().getQueryAsForm() != null)
          ...
          try {
            FileUtils.cleanDirectory(new File(decodedPath));
          }
          catch (IOException e) {
  ...
  public void doInit() {
    try {
      ...
      if (getClientDispatcher() == null) {
        ...
        if (isNegotiated() && !this.isDirectoryTarget() && !isFileTarget() && (this.originalRef != null)) {
  ...
  public Representation handle() {
    ...
    if (getMethod() == Method.DELETE) {
      try {
        ...
        result = (Representation) super.handleFromUser();
      }
