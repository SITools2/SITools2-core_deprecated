    ...
    this.store = new Ext.data.Store({
        ...
        listeners : {
            ...
            load : function (self, records, index) {
                if (records.length > 1) {
                    if (Ext.isEmpty(records[0].data.pubDate)) {
    ...
    function clickOnRow(self, rowIndex, e) {
        ...
        if (Ext.isEmpty(window) || Ext.isEmpty(window.SitoolsDesk)) {
            ...
            var windowConfig = {
                ...
                datasetName : config.datasetName,
                saveToolbar : true,
                urlPreferences : loadUrl.get('APP_URL') + AppUserStorage + "/" + DEFAULT_PREFERENCES_FOLDER + "/" + projectGlobal.projectName + "/" + config.datasetName + "/dataDetail",
                type : "dataDetail", 
                toolbarItems : [{
...
Ext.extend(sitools.user.component.openSearchResultFeed, Ext.grid.GridPanel, {
    ...
    formatTitle : function (value, p, record) {
        ...
        if (link !== undefined && link !== "") {
            return String.format('<div class="topic"><a href="{0}"><span class="rss_feed_title">{1}</span></a><br/><span class="author">{2}</span></div>', link, value, 
                    author);
        } else {
