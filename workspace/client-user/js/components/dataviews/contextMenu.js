...
/*
 * @include "goToSvaTaskWindow.js"
 * @include "../../viewDataDetail/viewDataDetail.js"
 * @include "resourcePluginParamsWindow.js"
 * @include "goToTaskWindow.js"
 * @include "../../../env.js"
 ...
 * @requires sitools.user.component.dataviews.goToSvaTaskWindow
 * @requires sitools.user.component.viewDataDetail
 * @requires sitools.user.component.dataviews.resourcePluginParamsWindow
 * @requires sitools.user.component.dataviews.goToTaskWindow
 */
...
//sitools.user.component.livegrid.ctxMenu = Ext.extend(Ext.menu.Menu, {
    ...
    constructor : function (config) {
        ...
        if (!this.svaStore) {
            this.svaStore = new Ext.data.JsonStore({
                root : "data",
                fields : [ {
                    name : "id",
                    type : "string"
                ...
                url : this.dataUrl + "/sva?filter=active"
            });
        }
        /* RESOURCE PART */
        ...
        this.svaStore.on('beforeload', this.onBeforeLoad, this);
        this.svaStore.on('load', this.onLoad, this);
	...
	/**
	 * Define the beforeload event of the resources and sva store :
	 * Call updateMenuItems.
	 */
    onBeforeLoad : function () {
        this.svaStore.baseParams = this.baseParams;
        /* RESOURCE PART */
	...
	/**
	 * Build the contextMenu : add the items entry for resources and SVA.
    ...
    updateMenuItems : function (loadedState, records) {
        ...
        this.svaPresent = false;
        this.removeAll();
        this.el.sync();
        this.menuSva = new Ext.menu.Menu();
        /* RESOURCE PART */
        ...
        if (loadedState) {
            this.svaStore.each(function (svaRec) {
                var sva = svaRec.data;
                this.svaPresent = true;
                var url = this.dataUrl + "/sva/" + sva.id + "/tasks";
                var icon;
                if (!Ext.isEmpty(sva.image)) {
                    icon = sva.image.url;
                }
                var userSyncChoice = "";
                if (sva.method == "GET" || sva.runType == "SVA_FORCE_RUN_ASYNC" || sva.runType == "SVA_FORCE_RUN_SYNC") {
                    userSyncChoice = sva.runType == "SVA_FORCE_RUN_ASYNC" ? "async" : "sync";
                    this.menuSva.add({
                        text : sva.label,
                        cls : 'x-btn-text-icon',
                        icon : icon,
                        listeners : {
                            scope : this,
                            click : function () {
                                this.svaClick(url, sva.method, userSyncChoice);
                            }
                        }
                    });
                } else {
                    userSyncChoice = sva.runType == "SVA_DEFAULT_RUN_ASYNC" ? "async" : "sync";
                    this.menuSva.add({
                        text : sva.label,
                        cls : 'x-btn-text-icon',
                        icon : icon,
                        menu : {
                            items : [ {
                                ...
                                icon : loadUrl.get('APP_URL') + "/common/res/images/icons/sva_exe_default.png"
                            }, {
                                text : i18n.get('label.synchrone'),
                                listeners : {
                                    scope : this,
                                    click : function () {
                                        this.svaClick(url, sva.method, "sync");
                                    }
                                },
                                icon : loadUrl.get('APP_URL') + "/common/res/images/icons/sva_exe_synchrone.png"
                            }, {
                                text : i18n.get('label.asynchrone'),
                                listeners : {
                                    scope : this,
                                    click : function () {
                                        this.svaClick(url, sva.method, "async");
                                    }
                                },
                                icon : loadUrl.get('APP_URL') + "/common/res/images/icons/sva_exe_asynchrone.png"
                            } ]
        ...
        //SVA are now deprecated
        /*if (this.svaPresent) {
            this.add({
                text : i18n.get('label.sva'),
                menu : this.menuSva,
                icon : loadUrl.get('APP_URL') + "/common/res/images/icons/SVA.png"
            });
        }*/
        /* RESOURCE PART */
    ...
    /**
     * the action used when click on a Sva menu. Request SVA tasks to the URL
     ...
     * 
     */
    svaClick : function (url, method, userSyncChoice) {
//        if(this.getNbRowsSelected() > DEFAULT_LIVEGRID_BUFFER_SIZE || this.getNbRowsSelected() != this.selections.length) {
//            Ext.Msg.alert(i18n.get('label.warning'),i18n.get("livegrid.selection.invalide.txt"));
//            return;
//        }
        Ext.getBody().mask("loading...", "x-mask-loading");
        var request = "";
        if (!(method == "POST" && userSyncChoice == "async")) {
            try {
        ...
        // var url = urlDetail + "/sva/" + sva.id + "/tasks';
        url += "?1=1" + request;

        // If Get => the SVA MUST be synchrone and then send a representation
        if (method == "GET") {
            window.open(url);
            Ext.getBody().unmask();
            return;
        } else {
            if (!Ext.isEmpty(userSyncChoice)) {
                url += "&run=" + userSyncChoice;
            }

            // First case : there is a selection :
            if (this.selections.length !== 0) {
                this._addSelectionsForSVA(this.selections, url);
            } else {
                if (this.origin == "Ext.ux.livegrid" || this.origin == "sitools.user.component.dataviews.tplView.TplView") {
                    this._addRequestForSVA(url);
                } else {
                    this._addSelectionsForSVA(this.grid.getStore().data.items, url);
                }
            }
    ...
    /**
     * Build the file for the SVA
     * 
     * @param selections :
     *            the selections of records
     * @param svaUrl :
     *            the url to send the file
     * @returns
     */
    _addSelectionsForSVA : function (selections, svaUrl) {
        var primaryKey = "";
        ...
        Ext.each(selections, function (rec) {
            var data = {};
            data[primaryKey] = rec.get(primaryKey);
            putObject.orderRecord.records.push(data);
        });
        var url;
        if (!Ext.isEmpty(userLogin)) {
            url = loadUrl.get('APP_USERSTORAGE_USER_URL').replace('{identifier}', userLogin);
        } else {
            url = "/tmp";
        }
        Ext.Ajax.request({
            url : loadUrl.get('APP_URL') + url,
            method : 'POST',
            scope : this,
            params : {
                filepath : "/tmp",
                filename : "SvaRecordDefinitionFile.json"
            },
            jsonData : putObject,
            success : function (ret) {
                ...
                if (!Json.success) {
                    ...
                    this._addRequestForSVA(svaUrl, {
                        params : {
                            urlFile : url + "/tmp/SvaRecordDefinitionFile.json"
                        }
                    });
                }
            },
    ...
    /**
     * Execute a POST request for a specific SVA.  
     * depending on result, it will open a new window, or build a {sitools.user.component.dataviews.goToSvaTaskWindow} Window.
     * @param {string} url
     * @param {} postObject the Object to post
     */
    _addRequestForSVA : function (url, postObject) {
        var config = {
            method : "POST",
            url : url,
            scope : this,
            success : function (response, opts) {
                try {
                    ...
                    var svaTask = json.SvaTaskModel;
                    if (!Ext.isEmpty(svaTask.urlResult)) {
                        window.open(svaTask.urlResult);
                    } else {
                        var windowSvaTask = new sitools.user.component.dataviews.goToSvaTaskWindow({
                            svaTask : svaTask
                        });
                        windowSvaTask.show();
                    }
    ...
    _onDetails : function () {
        ...
        var windowConfig = {
            ...
            datasetName : this.datasetName,
            saveToolbar : true,
            urlPreferences : loadUrl.get('APP_URL') + AppUserStorage + "/" + DEFAULT_PREFERENCES_FOLDER + "/" + projectGlobal.projectName + "/"
...
//			return this.grid.getSelectionModel().getAllSelections(false).length;
    ...
    handleResourceClick : function (resource, url, methods, runType, parameters) {
        ...
        if (showParameterBox) {
            var resourceWindow = new sitools.user.component.dataviews.resourcePluginParamsWindow({
                resource : resource,
                url : url,
                methods : methods,
                runType : runType,
     ...
     *            the liveGrid
     * @param method :
     *            the method for the sva : POST or GET
     * @param userSyncChoice :
     *            the user choice : "sync" || "async"
     * @param limit :
     *            the request limit
        ...
        var config = {
            ...
            success : function (response, opts) {
                ...
                if (!Ext.isEmpty(task.urlResult)) {
                    ...
                    var windowTask = new sitools.user.component.dataviews.goToTaskWindow({
                        task : task
                    });
                    windowTask.show();
                }
	...
	checkResourceParameters : function (resource, url, methods, runType, parameters) {
		...
		if (!Ext.isEmpty(maxThreshold)) {
            ...
            if (!Ext.isEmpty(maxThreshold.value) && maxThreshold.value != -1 && nbRows > maxThreshold.value) {
        ...
        if (!Ext.isEmpty(warningThreshold)) {
            //get the number of rows either from a selection or all the rows
            nbRows = (Ext.isEmpty(this.getNbRowsSelected()) || this.getNbRowsSelected() === 0) ? this.getNbGridRecords() : this.getNbRowsSelected();
            if (!Ext.isEmpty(warningThreshold.value) && warningThreshold.value != -1 && nbRows > warningThreshold.value) {
