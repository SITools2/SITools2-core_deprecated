...
//            url : '/sitools/portal/projects?media=json', 
            ...
            success : function (response) {
                try {
                    this.projects = response.responseJSON.data;
...
//            url : loadUrl.get('APP_URL') + loadUrl.get('APP_CLIENT_USER_URL') + '/tmp/langues.json',
            ...
            success : function (response) {
                this.languages = response.responseJSON.data;
                if (this.autoChainAjaxRequest) {
    ...
    initPreferences : function (cb) {
        if (Ext.isEmpty(userLogin)) {
            ...
            i18n.load(loadUrl.get('APP_URL') + '/res/i18n/' + LOCALE + '/gui.properties', callback);
            return;
        }
        var AppUserStorage = loadUrl.get('APP_USERSTORAGE_USER_URL').replace('{identifier}', userLogin);
        Ext.Ajax.request({
            scope : this,
            url : loadUrl.get('APP_URL') + AppUserStorage + "/" + DEFAULT_PREFERENCES_FOLDER + '/portal/portal?media=json',
//            url : '/sitools/userstorage/' + userLogin + "/" + DEFAULT_PREFERENCES_FOLDER + '/portal/portal?media=json',
            method : 'GET',
            success : function (response) {
                this.preferences = Ext.decode(response.responseText);
                if (!Ext.isEmpty(this.preferences.language)) {
                    LOCALE = this.preferences.language;
                }
            ...
            callback : function () {
                ...
                var languages = this.languages;
                var preferences = this.preferences;
                i18n.load(loadUrl.get('APP_URL') + '/res/i18n/' + LOCALE + '/gui.properties', function () {
                    // loadUrl.load('/sitools/client-user/siteMap', function (){
                    portal = new sitools.Portal(projects, languages, preferences);
                    // });
                });
